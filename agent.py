...
class DQN:
    def __init__(self, input_size, output_size, learning_rate, gamma, num_layers=3, num_heads=3, batch_size=64, optimizer_type='RMSProp'):
        """初始化DQN代理"""
    ...
    def _get_optimizer(self, optimizer_type, lr):
        ...
        if optimizer_type == 'Adam':
            # 推荐：适应性强，对噪声奖励鲁棒性好
            return optim.Adam(self.q_network.parameters(), lr=lr, weight_decay=1e-5)
        elif optimizer_type == 'RMSprop':
            # 原使用的优化器，适合非平稳目标
            return optim.RMSprop(self.q_network.parameters(), lr=lr)
        elif optimizer_type == 'AdamW':
            # Adam的改进版，更好的权重衰减正则化
            return optim.AdamW(self.q_network.parameters(), lr=lr, weight_decay=1e-4)
